		...
		try{
			...
			if($poloczenie->connect_errno!=0){
				...
				if($ile_takich_nickow>0){
					$wszystko_OK=false;
					$poloczenie->query("UPDATE  polecam SET polecam = 'nie' WHERE gra='$gra' AND uzytkownik='$uzytkownik'");
					$_SESSION['udane_oddanie_oceny']='<span style="color:yellow;font-size: 30px;"
						>Nie polecasz tej gry!</span>';
					header('Location:recka.php?tak='. $gra);
					exit();
				}					

				if($wszystko_OK==true){
					if($poloczenie->query("INSERT INTO polecam VALUES(NULL, '$gra', '$uzytkownik', 'nie')")){
						$_SESSION['udane_oddanie_oceny']='<span style="color:yellow;font-size: 30px;"
						>Nie polecasz tej gry!</span>';
						header('Location:recka.php?tak='. $gra);
					}else{
